...
import { Phone, Mail, Lock, User, ArrowRight, Loader2, AlertCircle } from "lucide-react";
...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style={{ background: "var(--bg-primary)" }}>
      <div className="absolute bottom-1/3 right-1/3 w-80 h-80 rounded-full blur-[120px] opacity-15" style={{ background: "var(--brand-500)" }} />
      <div className="w-full max-w-md relative z-10 animate-fade-in">
        ...
        <div className="card p-8">
          <h1 className="text-2xl font-bold text-center mb-2" style={{ color: "var(--text-primary)" }}>Create your account</h1>
          <p className="text-sm text-center mb-8" style={{ color: "var(--text-secondary)" }}>Start receiving AI-handled calls in 24 hours</p>
          ...
          <p className="text-sm text-center mt-6" style={{ color: "var(--text-muted)" }}>
            ...
            <Link href="/login" className="font-medium" style={{ color: "var(--brand-400)" }}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
